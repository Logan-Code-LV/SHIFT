...
export const AuthProvider = props => {
  ...
  function signinFree(username, password) {
    return new Promise((resolve, reject) => {
      api
        .loginFree(username, password)
        .then(data => {
...
export function useAuth() {
  const { signin, signinFree, signout, isAuthenticated } = useContext(
    AuthContext
  )
  const profile = api.getProfile() || {}

  return { signin, signinFree, signout, isAuthenticated, profile }
}
